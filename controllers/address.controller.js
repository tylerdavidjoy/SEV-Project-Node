...
exports.create = (req, res) => {
  ...
  Address.create(address, person_ID, (err, data) => {
    if (err)
      ...
      } else if(err.kind == "not_found_type") {
        res.status(400).send({
          message:
            err.message || "Could not find valid_value for type field."
        });
      } 
      else {
        res.status(500).send({
    ...
    Address.findById(id, (err, data) => {
      if (err) {
        if (err.kind == "not_found") {
          res.status(404).send({
            message:
              err.message || "Could not find address for ID " + id + "."
          });
        } else {
          res.status(500).send({
            message:
  ...
  Address.updateById(req.query.id, new Address(req.body), (err, data) => {
    if (err) {
      if (err.kind == "not_found") {
        res.status(404).send({
          message:
            err.message || "Could not find address for ID " + req.query.id + "."
        });
      } else {
        res.status(500).send({
