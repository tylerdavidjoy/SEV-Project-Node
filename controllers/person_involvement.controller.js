        ...
        Person_Involvement.findByPersonIdAndInvolvementId(person_id, involvement_id, (err, data) => {
            if (err) {
                if (err.kind == "not_found") {
                    res.status(404).send({
                        message:
                            err.message || "Could not find person_involvement for person_ID " + person_id + " and involvement_ID " + involvement_id
                    });
                } else {
                    res.status(500).send({
                        message:
    ...
    Person_Involvement.remove(person_id, involvement_id, (err, data) => {
        if (err) {
            if (err.kind == "not_found") {
                res.status(404).send({
                    message:
                        err.message || "Could not find person_involvement to delete for person_ID " + person_id + " and involvement_ID " + involvement_id
                });
            } else {
